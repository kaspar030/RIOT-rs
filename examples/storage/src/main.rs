...
async fn main() {
    ...
    // here, a literal can be used.
    storage::put("string_key", "string_value").await.unwrap();
    ...
    // getting an object. type needs to match.
    let cfg: Option<MyConfig> = storage::get("my_config").await.unwrap();
    if let Some(cfg) = cfg {
        info!("got cfg {:?}", cfg);
    }

    // getting a value as raw bytes returns probably not what you want due to the way
    // postcard works
    ...
    storage::put("some_raw_bytes", bytes).await.unwrap();
